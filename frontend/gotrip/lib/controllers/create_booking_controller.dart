...
class CreateBookingController extends GetxController {
  ...
  // Update both marker and address when location is selected
  Future<void> updateMarkerAndAddress(LatLng position) async {
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        pickupLocation.value = addressParts.where((part) => part.isNotEmpty).join(', ');
      } else {
        // Fallback if no address details are found
        pickupLocation.value = 'Selected location in Pokhara (${position.latitude.toStringAsFixed(4)}, ${position.longitude.toStringAsFixed(4)})';
      }
    } catch (e) {
