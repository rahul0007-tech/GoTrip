...
class DriverProfileController extends GetxController {
  ...
  var updated_at = ''.obs;
  var hasVehicle = false.obs;
  var tempProfileImage = Rxn<XFile>(); // Store temporary profile image
  ...
  void setTempProfileImage(XFile image) {
    tempProfileImage.value = image;
  }

  Future<void> updateProfileImage() async {
    if (tempProfileImage.value == null) return;

    try {
      final formData = dio.FormData.fromMap({
        'photo': await dio.MultipartFile.fromFile(
          tempProfileImage.value!.path,
          filename: tempProfileImage.value!.path.split('/').last,
        ),
      });

      final response = await httpClient.post(
        '/users/change-driver-profile-picture/',
        data: formData,
      );

      if (response.statusCode == 200) {
        photo.value = response.data['photo'] ?? photo.value;
        tempProfileImage.value = null; // Clear temporary image
        Get.snackbar(
          'Success',
